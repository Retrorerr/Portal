...
/// Pointer-emulation fallback only. Native `wl_touch` keeps exact coordinates
/// and lets the receiving toolkit own kinetic scrolling.
const TOUCHSCREEN_POINTER_SCROLL_GAIN: f64 = 0.55;
...
pub fn centralize(event: WindowEvent, backend: &mut WaylandBackend) -> CentralizedEvent {
    ...
    return match event {
        ...
        WindowEvent::Touch(Touch {
            ...
            if backend.touch_points.len() == 1 {
                backend.native_touch_active =
                    backend.native_touch_preferred && backend.compositor.touch.has_client();
                log::debug!(
                    "input.touch path={} pointer_fallback_gain={TOUCHSCREEN_POINTER_SCROLL_GAIN}",
                    if backend.native_touch_active {
                        "wl_touch"
                    } else {
                        "pointer"
                    }
            ...
            if backend.native_touch_active {
                return CentralizedEvent::Input(InputEvent::TouchDown {
        ...
        WindowEvent::Touch(Touch {
            ...
            if backend.native_touch_active {
                ...
                return CentralizedEvent::Input(InputEvent::TouchMotion {
                    ...
                    CentralizedEvent::Input(InputEvent::PointerAxis {
                        event: WinitMouseWheelEvent::from_touchscreen(
                            ...
                            MouseScrollDelta::PixelDelta(PhysicalPosition {
                                x: dx * TOUCHSCREEN_POINTER_SCROLL_GAIN,
                                y: dy * TOUCHSCREEN_POINTER_SCROLL_GAIN,
                            }),
                            phase,
                        ),
                    })
        ...
        WindowEvent::Touch(Touch {
            ...
            let native_protocol = backend.native_touch_active;
            if native_protocol {
                ...
                return CentralizedEvent::Input(InputEvent::TouchUp {
        ...
        WindowEvent::Touch(Touch {
            ...
            if backend.native_touch_active {
                backend.reset_touch_state();
                return CentralizedEvent::Input(InputEvent::TouchCancel {
