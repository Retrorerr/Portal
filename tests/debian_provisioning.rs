...
fn compatible_runtime_survives_artifact_revision_and_preserves_user_files() {
    ...
    // The image is still a complete Debian Forky installation, but the APK now
...
fn production_runtime_artifact_matches_manifest_and_archive_verification() {
    ...
    assert_eq!(
        artifact.sha256,
        "b2e96dda836c62f42bf3e28b799d7bedadcd99f8eb2410654ba0a23c7d750b20"
    );
    assert_eq!(artifact.compressed_bytes, 1129255504);
    assert!(artifact.url.starts_with("https://github.com/Retrorerr/Portal/releases/download/runtime-debian14-arm64-2026.09.14.2/"));
    let archive_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("target/portal-debian14-arm64-2026.09.14.2.tar.xz");
...
#[test]
fn runtime_builder_uses_forky_packages_and_publisher_validates_them() {
    // Debian supplies the current Forky desktop packages. Portal's Anland
    // KWin binary is staged separately by the APK and is not substituted into
    // the package archive.
    ...
    for guard in [
        ...
        assert!(
            ...
            "publisher missing Forky validation guard: {guard}"
        );
    }
}
