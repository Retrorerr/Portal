...
fn authoritative_display_state_pointer_and_cursor_bijection() {
    ...
    let mut state = AuthoritativeDisplayState::new(3392, 2400, 420, 120000);
    state.update_observed_surface_size((1696.0, 1200.0)); // 2x scale
    ...
    // Physical cursor placement for hotspot (10, 10)
    let scale = state.presentation_scale();
    let hotspot = (10.0, 10.0);
    let elem_phys = (
        phys_pt.x - hotspot.0 * scale.0,
        phys_pt.y - hotspot.1 * scale.1,
    );
    assert_near(elem_phys.0, 1000.0 - 20.0);
    assert_near(elem_phys.1, 800.0 - 20.0);

    // When rendered with scale.0 = 2.0, hotspot (10, 10) lands at exactly:
    let hotspot_landed = (
        elem_phys.0 + hotspot.0 * scale.0,
        elem_phys.1 + hotspot.1 * scale.1,
    );
...
fn fractional_225_scale_pointer_alignment_and_decoupling() {
    ...
    state.update_observed_surface_size((1130.0, 800.0));

    // Presentation scale maps the 1130x800 texture to 3392x2400 (~3.0x)
    let pres = state.presentation_scale();
    assert_near(pres.0, 3392.0 / 1130.0);
    assert_near(pres.1, 3.0);
    ...
    // Round-trip back to physical pixels is exact
    let back_br = tx.logical_to_physical(bottom_right);
    assert_near(back_br.x, 3392.0);
    assert_near(back_br.y, 2400.0);
}
