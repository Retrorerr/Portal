...
public:
     ...
     *  frame. The next work request is driven by RenderLoop's actual repaint
     *  scheduling, not by this completion signal. */
    void onConsumerReady();

    /** Set by the output layer immediately before present(). An empty damage
     *  region is an explicit NO_DAMAGE completion and must not queue pixels. */
    void setFrameDamage(bool damaged);
