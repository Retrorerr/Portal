...
/* Request one actual producer work item. The message carries a monotonically
 * increasing sequence and the current connection generation. It is coalesced
 * by the KWin output before calling this function; no request is generated by
 * a display tick alone. */
int request_frame(display_ctx *ctx);

/* Wait for the consumer to select the slot for the requested work item. The
 * eventfd is consumed exactly once here. Returns 1 when selected, 0 on a
 * timeout, and -1 after a disconnected consumer. */
int wait_buffer_ready(display_ctx *ctx, int timeout_ms);

/* Signal the consumer that a damaged frame is complete by sending one packet
 * (with the render fence, if any) on the dedicated fence channel. */
int  trigger_refresh(display_ctx *ctx);

/* Signal an explicit no-damage completion. The consumer cancels the selected
 * slot with -1 and never queues duplicate pixels. */
