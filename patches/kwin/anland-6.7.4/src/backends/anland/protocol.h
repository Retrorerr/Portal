...
/* Portal Anland protocol v3. The v3 extension is intentionally negotiated on
 * the control channel before any consumer fds are handed over. */
#define ANLAND_PROTOCOL_VERSION 3
#define ANLAND_FEATURE_FRAME_WORK (1u << 0)
#define ANLAND_FEATURE_NO_DAMAGE (1u << 1)
#define ANLAND_REQUIRED_FEATURES (ANLAND_FEATURE_FRAME_WORK | ANLAND_FEATURE_NO_DAMAGE)
...
#define DATA_MSG_FRAME_WANTED    105
#define DATA_MSG_BUFS_READY      200

/* Producer -> consumer fence-channel packet types. FRAME_DONE may carry one
 * SCM_RIGHTS render fence; NO_DAMAGE must carry no fd and is cancelled by the
 * consumer. Both packets carry the exact work identity. */
#define FENCE_MSG_FRAME_DONE    1
#define FENCE_MSG_NO_DAMAGE     2
