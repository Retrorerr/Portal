...
{
    if (qEnvironmentVariableIntValue("ANLAND_NO_DRM_DEVICE") == 1) {
        qCInfo(KWIN_ANLAND) << "ANLAND_NO_DRM_DEVICE=1; using surfaceless EGL";
        return nullptr;
    }
...
AnlandBackend::~AnlandBackend()
{
    ...
    // RenderDevice owns the shared EGL display used by EglBackend.
}
...
bool AnlandBackend::initialize()
{
    ...
    const bool noDrmDevice = qEnvironmentVariableIntValue("ANLAND_NO_DRM_DEVICE") == 1;
    if (!m_renderDevice && !noDrmDevice) {
...
{
    if (m_sceneEglDisplay) {
        return m_sceneEglDisplay;
    }
    return m_renderDevice ? m_renderDevice->eglDisplay() : nullptr;
}

void AnlandBackend::setSceneEglDisplayObject(EglDisplay *display)
{
    m_sceneEglDisplay = display;
}
...
bool AnlandBackend::requestFrameWork()
{
    ...
    const int requested = request_frame(m_display);
    if (requested != 1) {
...
bool AnlandBackend::notifyFramePresented(bool damaged)
{
    ...
    const int result = damaged ? trigger_refresh(m_display) : trigger_no_damage(m_display);
    m_consumerReady = false;
    if (result < 0) {
        return false;
    }
