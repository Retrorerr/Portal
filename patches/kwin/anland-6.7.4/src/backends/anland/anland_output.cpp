...
bool AnlandOutput::present(const QList<OutputLayer *> &layersToUpdate, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    m_frame = frame;
    const bool damaged = m_frameHasDamage;
    const bool handedToConsumer = m_backend->notifyFramePresented(damaged);
    // A new KWin work item is considered no-damage unless its output layer
    // explicitly reports rendered damage. This prevents an empty compositor
    // traversal from queueing a stale dmabuf merely because present() ran.
    m_frameHasDamage = false;
    if (handedToConsumer && damaged) {
...
void AnlandOutput::onConsumerReady()
{
    ...
    // Do not schedule here. A buffer-ready event only completes the previous
    // handoff; RenderLoop's own repaint requests are the sole source of the
    // next FRAME_WANTED message. This is what leaves a static desktop idle.
}

void AnlandOutput::setFrameDamage(bool damaged)
{
    m_frameHasDamage = damaged;
}
...
void AnlandOutput::resumeRendering()
{
    ...
    // uninhibit() schedules through RenderLoopPrivate directly. Emit the
    // public repaint boundary as well so the consumer gets a work request for
    // the initial post-reconnect frame.
