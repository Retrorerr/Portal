...
struct display_ctx {
    ...
    uint64_t active_frame_sequence;
    uint64_t active_frame_generation;
    bool     fallback;
...
int connect_to_deamon(display_ctx **out, const char *socket_path)
{
    ...
    struct ctrl_msg hdr = { .type = CTRL_MSG_PRODUCER_HELLO, .size = sizeof(struct protocol_hello) };
    struct protocol_hello hello = {
        .version = ANLAND_PROTOCOL_VERSION,
        .features = ANLAND_REQUIRED_FEATURES,
    };
    if (send_all(ctx->ctrl_fd, &hdr, sizeof(hdr)) < 0
        || send_all(ctx->ctrl_fd, &hello, sizeof(hello)) < 0)
        goto fail;

    struct ctrl_msg response;
    if (recv_all(ctx->ctrl_fd, &response, sizeof(response)) < 0)
        goto fail;
    if (response.type == CTRL_MSG_REJECT) {
        /* The reject payload is advisory; close this session fail-closed. */
        if (response.size > 0 && response.size <= 4096) {
            uint8_t reject_payload[4096];
            if (recv_all(ctx->ctrl_fd, reject_payload, response.size) < 0)
                goto fail;
        }
        goto fail;
    }
    if (response.type != CTRL_MSG_SCREEN_INFO || response.size != sizeof(struct screen_info))
        goto fail;

    struct screen_info si;
    if (recv_all(ctx->ctrl_fd, &si, sizeof(si)) < 0)
        goto fail;
...
int request_frame(display_ctx *ctx)
{
    if (ctx->fallback) {
        return 0;
    }

    /* There is only one selected/rendering work item at a time. The KWin
     * output normally coalesces before reaching this boundary; keep the C
     * transport defensive so a duplicate cannot overwrite its fence identity. */
    if (ctx->active_frame_sequence != 0)
        return 0;

    ctx->next_frame_sequence++;
    if (ctx->next_frame_sequence == 0)
        ctx->next_frame_sequence = 1;
    ctx->active_frame_sequence = ctx->next_frame_sequence;
    ctx->active_frame_generation = ctx->connection_generation;

    struct data_msg hdr = {
        .type = DATA_MSG_FRAME_WANTED,
        .size = sizeof(struct frame_wanted),
    };
    struct frame_wanted wanted = {
        .sequence = ctx->active_frame_sequence,
        .generation = ctx->active_frame_generation,
    };
    if (send_all(ctx->data_fd, &hdr, sizeof(hdr)) < 0
        || send_all(ctx->data_fd, &wanted, sizeof(wanted)) < 0) {
        enter_fallback(ctx);
        return -1;
    }
    return 1;
...
int wait_buffer_ready(display_ctx *ctx, int timeout_ms)
{
    if (ctx->fallback)
        return -1;

    struct pollfd pfd = { .fd = ctx->buf_ready_efd, .events = POLLIN };
    int ret;
    do {
    ...
    uint64_t value;
    if (read(ctx->buf_ready_efd, &value, sizeof(value)) != (ssize_t)sizeof(value)) {
        enter_fallback(ctx);
        return -1;
    }
    return 1;
...
static int trigger_frame(display_ctx *ctx, uint32_t type)
{
    if (ctx->fallback) {
        if (ctx->pending_render_fence >= 0) {
            close(ctx->pending_render_fence);
        ...
        ctx->active_frame_sequence = 0;
        ctx->active_frame_generation = 0;
        return 0;
    }

    if (ctx->active_frame_sequence == 0 || ctx->active_frame_generation == 0)
        return -1;

    struct fence_msg packet = {
        .type = type,
        .reserved = 0,
        .sequence = ctx->active_frame_sequence,
        .generation = ctx->active_frame_generation,
    ...
    if (type == FENCE_MSG_FRAME_DONE && ctx->pending_render_fence >= 0) {
    ...
    } else if (type == FENCE_MSG_NO_DAMAGE && ctx->pending_render_fence >= 0) {
        close(ctx->pending_render_fence);
        ctx->pending_render_fence = -1;
    }
...
{
    return trigger_frame(ctx, FENCE_MSG_FRAME_DONE);
}

int trigger_no_damage(display_ctx *ctx)
{
    return trigger_frame(ctx, FENCE_MSG_NO_DAMAGE);
}
