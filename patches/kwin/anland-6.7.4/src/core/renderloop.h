...
Q_SIGNALS:
    ...
    /**
     * This signal is emitted whenever a repaint request is accepted or
     * deferred by the render loop. Anland uses it to request a consumer buffer
     * before the compositor begins the corresponding frame; it does not make
     * a display tick into implicit render work.
     */
    void repaintScheduled(RenderLoop *loop);
