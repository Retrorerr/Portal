// Diagnostic-only in-process SIGUSR2 stack capture for kcminit_startup.
//
// This preload intentionally installs one handler only: SIGUSR2.  It does
// not replace the production crash handler's SIGSEGV/SIGABRT behavior.  The
// signal handler is best effort because backtrace() is not async-signal-safe;
// it is used here only after the process has been observed waiting in poll.
#define _GNU_SOURCE
...
static char stack_path[512] = "/var/lib/localdesktop/kcminit-stack.log";
...
static void capture_signal(int signal_number, siginfo_t *info, void *raw_context) {
    ...
    write_text(fd, "kcminit-stack-probe signal=");
    ...
    // This is deliberately best-effort.  The raw register/map evidence above
    // is written first so a loader or allocator re-entry cannot erase it.
    void *frames[64];
...
static void write_start_marker(void) {
    ...
    write_text(fd, "kcminit-stack-probe-start role=exec-parent pid=");
    write_decimal(fd, (unsigned long)getpid());
    write_text(fd, " nonce=");
    const char *nonce = getenv("LOCALDESKTOP_KCMINIT_STACK_NONCE");
    if (nonce != NULL) {
        write_text(fd, nonce);
    } else {
        write_text(fd, "unset");
...
__attribute__((constructor)) static void install_probe(void) {
    const char *configured_path = getenv("LOCALDESKTOP_KCMINIT_STACK_LOG");
